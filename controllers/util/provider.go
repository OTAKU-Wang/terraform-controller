...
const (
	...
	EnvOpenstackUser       = "OS_USERNAME"
	EnvOpenstackPassword   = "OS_PASSWORD"
	EnvOpenstackTenantName = "OS_TENANT_NAME"
	EnvOpenstackAuthUrl    = "OS_AUTH_URL"
	EnvOpenstackRegion     = "OS_REGION_NAME"
...
type OpenstackCredentials struct {
	OpenstackUser       string `yaml:"openstackUser"`
	OpenstackTenantName string `yaml:"openstackTenantName"`
	OpenstackPassword   string `yaml:"openstackPassword"`
	OpenstackAuthUrl    string `yaml:"openstackAuthUrl"`
}
...
func GetProviderCredentials(ctx context.Context, k8sClient client.Client, namespace, providerName string) (map[string]string, error) {
	...
	case "Secret":
		...
		case string(Openstack):
			var cred OpenstackCredentials
			if err := yaml.Unmarshal(secret.Data[secretRef.Key], &cred); err != nil {
				klog.ErrorS(err, errConvertCredentials, "Name", secretRef.Name, "Namespace", secretRef.Namespace, "secret", string(secret.Data[secretRef.Key]))
				return nil, errors.Wrap(err, errConvertCredentials)
			}
			return map[string]string{
				EnvOpenstackUser:       cred.OpenstackUser,
				EnvOpenstackPassword:   cred.OpenstackPassword,
				EnvOpenstackAuthUrl:    cred.OpenstackAuthUrl,
				EnvOpenstackTenantName: cred.OpenstackTenantName,
				EnvOpenstackRegion:     region,
			}, nil
		}
	default:
